...
document.addEventListener('DOMContentLoaded', () => {
  ...
  Promise.all([
    ...
    try {
      ...
      if (top && top.r30 > 5) {
        if (p) p.textContent = `+${top.r30}% 지출 증가 예상`;
    ...
    try {
      ...
      if (card && top && top.r30 > 5) {
        const titleEl = card.querySelector('p.font-headline-md');
        if (titleEl) titleEl.textContent = `${top.name} 가격 30일 뒤 ${top.r30}% 상승 예상!`;
        ...
        const descEl = card.querySelector('p.font-body-sm');
        if (descEl) descEl.textContent = msg;
      }
    } catch (e) {
      console.error('[홈] 재고 확보 알림 카드 렌더링 실패:', e);
    }
    ...
    try {
      ...
      if (bomH) {
        ...
        if (grid) {
          const tpl = grid.firstElementChild ? grid.firstElementChild.cloneNode(true) : null;
          if (tpl) {
      ...
      console.error('[홈] 메뉴별 원가 BOM 섹션 렌더링 실패:', e);
      const grid = document.querySelector('h3')?.parentElement?.parentElement?.querySelector('.grid');
      if (grid) {
        grid.innerHTML = `
          <div class="glass-card rounded-xl p-md text-center text-on-surface-variant col-span-full">
            <span class="material-symbols-outlined text-error text-[24px] mb-xs">error</span>
            <p class="font-label-sm">메뉴 정보를 불러오지 못했습니다.</p>
          </div>
        `;
      }
    }
