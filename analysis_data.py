...
def _cov_verdict(nominal, cov):
    # 포함률 판정 문구. 값을 문장에 박아 두면 재학습 때마다 대시보드가 거짓말을 한다.
    if nominal is None or cov is None:
        return "포함률을 아직 계산하지 못했습니다."
    gap = cov - nominal
    if gap >= 0:
        tail = (f"{gap}%p 넘침은 구간을 조금 넉넉히 잡았다는 뜻이라 안전한 방향의 오차입니다."
                if gap else "목표를 정확히 맞췄습니다.")
    else:
        tail = (f"{-gap}%p 모자랍니다. 약속한 만큼 담지 못했다는 뜻이라 구간을 넓혀야 합니다.")
    return f"목표 {nominal}%에 실측 {cov}%. {tail}"
...
def reliability():
    ...
    return {
        ...
        "interval": {
            ...
            "verdict": _cov_verdict(iv.get("nominal"), iv.get("coverage_avg")),
        },
        ...
        "grades": [
            ...
            {"근거": "예측구간 포함률", "표본": f"{iv.get('cqr_picked', '')} 조합",
             "등급": "강", "이유": f"약속한 확률({iv.get('nominal')}%)과 실제 결과({iv.get('coverage_avg')}%)를 "
                                  "직접 맞대 본 값입니다. 모델이 스스로의 불확실성을 재는 능력을 검증합니다."},
            {"근거": "방향 적중률", "표본": f"{h30.get('n', 0):,}건",
             "등급": "중", "이유": f"30일 {h30.get('dir_acc')}%는 쓸 만하지만 7일 {h7.get('dir_acc')}%는 "
                                  "동전 던지기와 큰 차이가 없습니다. 단기 방향은 아직 믿을 수준이 아니라고 봅니다."},
            {"근거": "실전 적중률", "표본": "유효 0건",
