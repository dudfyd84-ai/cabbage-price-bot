...
> 실제 키 값은 코드·문서에 넣지 않는다. 아래 항목을 각자 발급받아 Cloudtype 환경변수/GitHub Secret으로만 등록한다.

| Key | Value |
|-----|-------|
| `KAMIS_KEY` | `<KAMIS에서 발급받은 인증키>` |
| `KAMIS_ID` | `<KAMIS 가입 이메일>` |
| `ASOS_KEY` | `<기상청 ASOS 서비스키>` |
| `GARAK_ID` | `<가락시장 공공데이터 id>` |
| `GARAK_PASSWD` | `<가락시장 공공데이터 비밀번호>` |
